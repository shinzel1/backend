...
$stmt = $pdo->prepare("
    SELECT i.id, i.filename, i.filepath, i.uploaded_at,
           COALESCE(r.name, b.title, rc.title) AS entity_name,
           CASE 
...
<head>
    ...
    <style>
        .drop-zone { border: 2px dashed #0d6efd; border-radius: 10px; padding: 30px; text-align: center; cursor: pointer; }
        .drop-zone.dragover { background: #e9f5ff; border-color: #0a58ca; }
    ...
    <form action="" method="post" enctype="multipart/form-data" class="card p-4 shadow-sm bg-white">
        ...
        <div class="mb-3">
            ...
            <select name="entity_type" id="entity_type" class="form-control" required>
                <option value="restaurant" <?= $image['entity_type'] === 'restaurant' ? 'selected' : '' ?>>Restaurant</option>
        ...
        <div class="mb-3">
            ...
            <select name="entity_id" id="entity_id" class="form-control" required>
                <option value="<?= (int)$image['entity_id'] ?>" selected><?= htmlspecialchars($image['entity_name']) ?></option>
            </select>
        </div>
